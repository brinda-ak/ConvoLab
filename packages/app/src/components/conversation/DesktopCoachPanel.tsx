...
function getInsightType(content: string): 'positive' | 'warning' | 'observation' {
  ...
  if (
    ...
    lower.includes('could come across') ||
    lower.includes('tone') ||
...
function InsightCard({ message }: { message: Message | AsideMessage }) {
  ...
  return (
    <div
      className={`${bg} border ${border} rounded-xl p-4 hover:border-opacity-80 transition-colors`}
    >
      <div className="flex items-start gap-2">
        ...
        <div
          className={`flex-1 text-sm ${text} leading-relaxed [&_p]:mb-1 [&_p:last-child]:mb-0 [&_strong]:font-bold [&_strong]:text-[#1A1A1A] dark:[&_strong]:text-[#EBEBEB]`}
        >
